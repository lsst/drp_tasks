...
- the point-spread function model model (`lsst.afw.detection.Psf`)
...
The tasks outputs are an `lsst.afw.image.ExposureF` that aggregates the calibration models and the `~lsst.afw.table.SourceCatalog` of single-visit measurements.
...
- the catalog of measurements (``source_detector``)
- a separate catalog of deblended footprints (``source_detector_footprints``)
